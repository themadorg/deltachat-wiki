...
description: Technical details for bot developers.
category: Bot
---
...
Here's a [list of all available events](./events) with descriptions of their meaning.
...
One useful example for an event and its payload would be [`DC_EVENT_INCOMING_MSG`](./events), the event emitted when a new message for an existing chat has arrived.
...
> **Note**: the high-level bot frameworks handle the `DC_EVENT_INCOMING_MSG` for you and provide you the incoming message data, this is only mentioned to explain the background.
