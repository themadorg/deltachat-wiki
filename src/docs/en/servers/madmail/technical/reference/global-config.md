...
Internet hostname of this mail server. Typically FQDN is used. It is recommended to make sure domain specified here resolved to the public IP of the server.
...
You can also specify other configuration options such as cipher suites and TLS version. See maddy-tls(5) for details. Madmail uses reasonable cipher suites and TLS versions by default so you generally don't have to worry about it.
...
This is optional block that specifies various TLS-related options to use when making outbound connections. See TLS client configuration for details on directives that can be used in it. Madmail uses reasonable cipher suites and TLS versions by default so you generally don't have to worry about it.
...
**Note:** Madmail does not perform log files rotation, this is the job of the logrotate daemon. Send SIGUSR1 to Madmail process to make it reopen log files.
