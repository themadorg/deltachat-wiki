---
title: Messenger
description: Messenger documentation for webxdc.
...
Webxdc apps are attached to a chat message using the [container file](./format) format,
...
- MUST serve all resources from the [webxdc container file](./format) 
...
- MUST support `<input type="file">` to allow importing of files;
  see [`sendToChat()`](../specification/sendToChat) for a way to export files. 
